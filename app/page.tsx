    ...
    <main className="workspace">
      <header className="topbar">
        ...
        <nav>
          ...
          <a href="#watchlist">Watchlist <b>{watchlist.length}</b></a>
      ...
      <section className="hero workspaceHero">
        <div>
          <div className="eyebrow">SCOUTING DESK / TODAY</div>
          <h1>Your scouting schedule, <span className="accent">automated.</span></h1>
          <p>Track the players you care about. Scoutboard finds the matches around them and turns your list into a personal match calendar.</p>
        </div>
        <div className="heroStat">
          <span className="eyebrow">WATCHING</span>
          <strong>{watchlist.length}</strong>
          <small>players on your board</small>
        </div>
      </section>

      <section className="commandBar">
        <button className="primaryButton" onClick={() => setShowSearch(true)}>+ Add player</button>
        <a className="ghostButton" href="#watchlist">Open watchlist</a>
        <a className="ghostButton" href="#calendar">View calendar</a>
        {notice && <span className="notice">{notice}</span>}
      </section>

      <section id="watchlist" className="sectionBlock">
        ...
        {watchlist.length === 0 ? (
          <article className="emptyCard">
            ...
            <p>Add the first player you want to follow. Their club becomes the starting point for the match radar.</p>
            ...
            {watchlist.map((player, index) => (
              <article className="playerCard" key={player.id}>
                <div className="cardTop">
                  ...
                  <button className="iconButton" aria-label={`Remove ${player.name}`} onClick={() => removePlayer(player.id)}>×</button>
                </div>
      ...
      <section className="splitGrid">
        <article id="calendar" className="featureCard">
          <div className="eyebrow">MATCH RADAR</div>
          <h2>Upcoming matches</h2>
          <p>Add a player and Scoutboard will resolve their club fixtures into a focused scouting calendar.</p>
          <div className="miniTimeline">
            <div><span>FRI 11</span><strong>Barcelona vs Real Madrid</strong><em>16:00</em></div>
            <div><span>SAT 12</span><strong>PSG vs Marseille</strong><em>20:45</em></div>
            <div><span>SUN 13</span><strong>Arsenal vs Chelsea</strong><em>16:30</em></div>
          </div>
        </article>
        <article className="featureCard">
          <div className="eyebrow">IPHONE CALENDAR</div>
          <h2>One calendar for your scouting days.</h2>
          <p>We’ll expose a private RFC 5545 feed so your iPhone Calendar can subscribe once and keep receiving schedule changes.</p>
          <div className="calendarPreview"><span>◎</span><div><strong>Scoutboard / My Players</strong><small>Private calendar feed</small></div></div>
        </article>
      </section>
      ...
      {showSearch && (
        <div className="modalBackdrop" onMouseDown={() => setShowSearch(false)}>
          <section className="searchModal" onMouseDown={(event) => event.stopPropagation()}>
            <div className="modalHeader"><div><div className="eyebrow">PLAYER DISCOVERY</div><h2>Add a player</h2></div><button className="iconButton" onClick={() => setShowSearch(false)}>×</button></div>
            <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search player or club..." />
            <div className="searchResults">
              {results.map((player) => {
                const alreadyAdded = watchlist.some((item) => item.id === player.id);
                return <button className="resultRow" key={player.id} onClick={() => addPlayer(player)} disabled={alreadyAdded}>
              ...
              {results.length === 0 && <div className="noResults">No matching player in the demo dataset.</div>}
            </div>
            <div className="modalFoot"><span>Demo data for UI development</span><span className="mono">Provider API → next</span></div>
          </section>
        </div>
      )}
    </main>

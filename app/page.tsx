    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/players/search?${params.toString()}`, {
          ...
          headers: {
            'x-scoutboard-gemini-key': geminiKey,
            ...(fixtureKey ? { 'x-scoutboard-api-key': fixtureKey } : {}),
          },
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Player discovery is unavailable.');

...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      setSearchError('');
      try {
        const response = await fetch(`/api/players/search?q=${encodeURIComponent(term)}`, { signal: controller.signal });
      ...
      {showSearch && (
        <div className="modalBackdrop" onMouseDown={() => setShowSearch(false)}>
          <section className="searchModal" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search any real player..." />
            <div className="searchResults">
              {!query.trim() && <div className="noResults">Type at least 2 characters to search the live player database.</div>}
              {searching && <div className="noResults">Searching live data…</div>}
              {searchError && <div className="searchError">{searchError}<small>Check API_FOOTBALL_KEY in your local .env.local.</small></div>}
              ...
              {!searching && !searchError && query.trim().length >= 2 && results.length === 0 && <div className="noResults">No player results.</div>}
            </div>

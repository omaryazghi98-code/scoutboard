...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      const unresolved = (Array.isArray(parsed) ? parsed : []).filter((player) => player.sourceProvider === 'gemini' && !player.teamId && player.team && player.team !== 'Unknown club');
  ...
  return <main className="workspace"><header className="topbar"><a className="brand" href="/">SCOUTBOARD <span>0.1</span></a><nav><a className="navActive" href="/">Board</a><a href="/watchlist">Watchlist <b>{watchedCount}</b></a><a href="/calendar">Calendar</a><a href="/settings">Settings</a></nav></header>
    ...
    {showSearch && <div className="modalBackdrop" onMouseDown={() => setShowSearch(false)}><section className="searchModal" onMouseDown={(event) => event.stopPropagation()}><div className="modalHeader"><div><div className="eyebrow">LIVE WEB DISCOVERY / GEMINI</div><h2>Add a player</h2></div><button className="iconButton" onClick={() => setShowSearch(false)}>×</button></div><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search any football player..." /><div className="searchResults">{!query.trim() && <div className="noResults">Type at least 2 characters to search.</div>}{searching && <div className="noResults">Gemini is searching…</div>}{searchError && <div className="searchError">{searchError}<small>Open Settings to configure Gemini.</small></div>}{!searching && !searchError && results.map((player) => <button className="resultRow" key={`${player.id}-${player.team}`} onClick={() => addPlayer(player)} disabled={watchlist.some((item) => playerKey(item) === playerKey(player))}><span className="resultAvatar">{player.name.split(' ').map((part) => part[0]).slice(0, 2).join('')}</span><span className="resultIdentity"><strong>{player.name}</strong><small>{player.team} · {player.position} · {player.nationality}</small></span><span className="resultAction">{watchlist.some((item) => playerKey(item) === playerKey(player)) ? 'ADDED' : '+ ADD'}</span></button>)}{!searching && !searchError && query.trim().length >= 2 && results.length === 0 && <div className="noResults">No player results.</div>}</div><div className="modalFoot"><span>Powered by Gemini</span><a className="mono" href="/settings">Configure AI</a></div></section></div>}
  </main>;
}

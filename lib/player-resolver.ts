...
type Candidate = Omit<ResolvedPlayer, 'provider' | 'sourceProvider' | 'identityConfidence' | 'identitySources'> & { source: 'footballdata' | 'thesportsdb' };
...
function nameScore(name: string, query: string) {
  ...
  const parts = b.split(' ');
  return Math.round((parts.filter((part) => a.includes(part)).length / parts.length) * 70);
}

function fromFootballdata(row: any): Candidate | null {
  ...
  const rawTeamId = team?.id ?? team?.team_id;
  return {
    id: String(id), providerId: String(id), name: String(name),
    team: String(team?.name ?? team?.team_name ?? 'Unknown club'),
    teamId: rawTeamId == null ? null : String(rawTeamId),
    ...
    sourceUrl: null, source: 'footballdata',
  };
}
...
async function footballCandidates(query: string, apiKey?: string) {
  ...
  try {
    ...
    const candidates = (Array.isArray(rows) ? rows : [])
      .map(fromFootballdata)
      .filter((candidate): candidate is Candidate => Boolean(candidate))
      .sort((a, b) => nameScore(b.name, query) - nameScore(a.name, query))
      .slice(0, 4);
    return Promise.all(candidates.map(async (candidate) => {
      if (candidate.teamId) return candidate;
      try { return fromFootballdata(await getPlayerFootballdata(candidate.providerId, apiKey)) || candidate; } catch { return candidate; }
    }));
  } catch { return []; }
}
...
async function sportsDbCandidates(query: string) {
  try {
    ...
    const candidates = rows.filter((row) => row.idPlayer && row.strPlayer).sort((a, b) => nameScore(b.strPlayer || '', query) - nameScore(a.strPlayer || '', query)).slice(0, 1);
    return Promise.all(candidates.map(async (row) => {
      ...
      return {
        ...
        // TheSportsDB IDs are not fixture-provider IDs. Never use them as teamId.
        teamId: null,
        teamLogo: null,
...
function mergeCandidates(query: string, football: Candidate[], sports: Candidate[]) {
  const merged = new Map<string, Candidate & { sources: string[]; score: number; sportsTeam?: string }>();
  for (const candidate of [...football, ...sports]) {
    ...
    if (!existing) {
      merged.set(key, { ...candidate, sources: [candidate.source], score, sportsTeam: candidate.source === 'thesportsdb' ? candidate.team : undefined });
      continue;
    }
    existing.sources = Array.from(new Set([...existing.sources, candidate.source]));
    existing.score = Math.max(existing.score, score);
    if (candidate.source === 'thesportsdb' && candidate.team !== 'Unknown club') existing.sportsTeam = candidate.team;
    if (candidate.source === 'thesportsdb' && candidate.team !== 'Unknown club') existing.team = candidate.team;
    if (!existing.teamId && candidate.source === 'footballdata' && candidate.teamId) existing.teamId = candidate.teamId;
...
export async function resolvePlayers(query: string, options: { footballApiKey?: string; geminiApiKey?: string; geminiModel?: string }) {
  ...
  // Gemini is only a disambiguation fallback. It never supplies the canonical club ID.
  if (!merged.length && options.geminiApiKey) {
    try {
      ...
      const discovered = await Promise.all(names.map(async (name) => ({ football: await footballCandidates(name, options.footballApiKey), sports: await sportsDbCandidates(name) })));
  ...
  return Promise.all(merged.slice(0, 8).map(async (candidate) => {
    let teamId = candidate.teamId ? String(candidate.teamId) : null;
    let team = candidate.team;
    let teamLogo = candidate.teamLogo;
    // Resolve the selected canonical club name against the fixture provider so IDs
    // always belong to the provider that will later fetch the match schedule.
    if (!teamId && team !== 'Unknown club' && options.footballApiKey) {
      ...
      if (resolved) { team = resolved.name; teamId = resolved.id; teamLogo = resolved.logo || teamLogo; }
    }
    const hasBothSources = candidate.sources.includes('footballdata') && candidate.sources.includes('thesportsdb');
    return {
      ...
      identityConfidence: hasBothSources && Boolean(teamId) ? 'high' as const : 'medium' as const,
